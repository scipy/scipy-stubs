...
_FloatT0 = TypeVar("_FloatT0", bound=npc.floating | float, default=np.float64 | float)
...
_Function00: TypeAlias = Callable[Concatenate[_FloatT0, ...], onp.ToFloat]
_Function11: TypeAlias = Callable[Concatenate[onp.Array1D[_FloatT], ...], onp.ToFloat1D]
_Function22: TypeAlias = Callable[Concatenate[onp.Array2D[_FloatT], ...], onp.ToFloat2D]
_FunctionNN: TypeAlias = Callable[Concatenate[onp.ArrayND[_FloatT], ...], onp.ToFloatND]
...
@type_check_only
class _HessianResult(_RichResult, Generic[_FloatT_co, _ShapeT_co]):
    status: onp.Array[_ShapeT_co, np.int32]
    error: onp.Array[_ShapeT_co, _FloatT_co]
    nfev: onp.Array[_ShapeT_co, np.int32]
    success: onp.Array[_ShapeT_co, np.bool_]
    ddf: onp.Array[_ShapeT_co, _FloatT_co]

###

@overload  # 0-d float64
def derivative(
    f: _Function00[np.float64] | _Function00[float],
    x: float | np.float64 | npc.integer,
    *,
...
@overload  # 2-d <unknown>
def derivative(
    f: _Function22[_FloatT],
    ...
    *,
    ...
    step_factor: onp.ToFloat = 2.0,
    step_direction: onp.ToJustInt | onp.ToJustInt1D = 0,
    preserve_shape: bool = False,
    callback: Callable[[_DerivativeResultND[_FloatT, tuple[int]]], _Ignored] | None = None,
) -> _DerivativeResultND[_FloatT, tuple[int, int]]: ...
